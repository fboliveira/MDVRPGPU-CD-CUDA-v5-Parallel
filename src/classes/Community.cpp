...
Community::Community(MDVRPProblem* problem, AlgorithmConfig* config,
...
vector<Subpopulation> Community::getSubpopsConst() const {
	return this->subpops;
}
...
void Community::setSubpopsPool(vector<Subpopulation> subpopsPool) {
...
void Community::setEliteGroup(EliteGroup* eliteGroup) {
...
void Community::setConfig(AlgorithmConfig* config) {
	this->config = config;
}
...
void Community::setProblem(MDVRPProblem* problem) {
...
void Community::createSubpopulations() {
	...
	for (int depot = 0; depot < this->getProblem()->getDepots(); ++depot) {
		Subpopulation subpop = Subpopulation(this->getProblem(),
				this->getConfig(), this->getEliteGroup(), depot);
...
void Community::pairingRandomly() {
	...
	for_each(this->getSubpops().begin(), this->getSubpops().end(),
...
void Community::evaluateSubpops(bool firstEvaluation) {
	...
	if (firstEvaluation
			|| this->getConfig()->getProcessType()
					== Enum_Process_Type::MONO_THREAD)
...
void Community::printEvolution() {
	...
	if (this->getConfig()->getProcessType() == Enum_Process_Type::MONO_THREAD) {
		...
		printf("%lu; %.2f; %.2f; %.2f; %.2f\n",
				...
				Util::diffTimeFromStart(
						this->getProblem()->getMonitor().getStart()),
				this->getEliteGroup()->getBest().getTotalCost(),
				this->getProblem()->getBestKnowSolution(), gap);
	} else {

		for_each(this->getEvolution().begin(), this->getEvolution().end(),
				[&] (typedef_evolution & evol) {
...
bool Community::isStopCriteriaMet() {
	...
	if (this->getConfig()->getStopCriteria() == NUM_GER) {
		if (this->getProblem()->getMonitor().getGeneration()
				> this->getConfig()->getNumGen()) {
			result = true;
		}
	} else {
		...
		if (Util::diffTimeFromStart(this->getProblem()->getMonitor().getStart())
				...
				|| (this->getConfig()->getMaxTimeWithoutUpdate() > 0
						&& Util::diffTimeFromStart(
								this->getProblem()->getMonitor().getLastTimeUpdate())
...
void Community::writeLogToFile() {
	...
	float gap = Util::calculateGAP(cost,
			this->getProblem()->getBestKnowSolution());
	...
	fprintf(fp, "%.2f;%.2f;%.2f;%.2f;%.2f\n",
			Util::diffTimeFromStart(
					this->getProblem()->getMonitor().getStart()),
			difftime(this->getProblem()->getMonitor().getLastTimeUpdate(),
					this->getProblem()->getMonitor().getStart()), cost,
			this->getProblem()->getBestKnowSolution(), gap);
...
void Community::evolve() {
	...
	if (this->getConfig()->getProcessType() == Enum_Process_Type::MONO_THREAD) {
		...
		for (auto ite = this->getSubpops().begin();
				ite != this->getSubpops().end(); ++ite) {
			(*ite).evolve();
		...
		while (this->getProblem()->getMonitor().isTerminated() == false) {
			...
			if (this->getProblem()->getMonitor().isCopyFromSubpopsRequested()) {
				...
				//printf("Copiado pool\n");
				this->getProblem()->getMonitor().setCopyFromSubpopsAllowed(
						true);
				this->getProblem()->getMonitor().setCopyFromSubpopsRequested(
						false);
...
void Community::updateBestIndividuals() {
	...
	if (this->getConfig()->getProcessType() == Enum_Process_Type::MONO_THREAD) {
		...
		for (auto depot = 0; depot < this->getSubpops().size(); ++depot) {
			this->getSubpops().at(depot).setBest(
					this->getEliteGroup()->getBest().getIndividuals().at(
							depot));
		}
		...
		while (this->getProblem()->getMonitor().isTerminated() == false) {
			...
			for (auto depot = 0; depot < this->getSubpopsPool().size();
					++depot) {
				this->getSubpopsPool().at(depot).setBest(
						this->getEliteGroup()->getBest().getIndividuals().at(
								depot));
			}
...
void Community::checkEvolution() {
	...
	if (this->getConfig()->getProcessType() == Enum_Process_Type::MONO_THREAD) {
		// Print evolution
		if (this->getConfig()->isDisplay()
				|| (this->getProblem()->getMonitor().getGeneration() % 100)
						== 0)
			// Print evolution
			this->printEvolution();
		...
		evolution.cost = cost;
		evolution.time = Util::diffTimeFromStart(
				this->getProblem()->getMonitor().getStart());

		auto lastPosition = this->getEvolution().insert_after(
				this->getEvolution().before_begin(), evolution);
		...
		while (this->getProblem()->getMonitor().isTerminated() == false) {
			...
			if (Util::diffTimeFromStart(readTime) >= 1) {
				...
				if (Util::isBetterSolution(newCost, cost)) {
					...
					lastPosition = this->getEvolution().insert_after(
							lastPosition, evolution);
...
void Community::evaluateSubpopsParallelOLD() {
	...
	while (this->getProblem()->getMonitor().isTerminated() == false) {
		...
		for (auto sId = 0; sId < this->getSubpopsPool().size(); ++sId) {
			...
			for (auto pId = 0;
					pId < this->getSubpopsPool().at(sId).getPairing().size();
					++pId) {

				//printf("Start Eval: S = %d\tP = %d\n", sId, pId);

				IndividualsGroup individuals = IndividualsGroup(
						this->getProblem(), this->getConfig(),
						this->getSubpopsPool().at(sId).getDepot());

				// Copy individuals
				for (size_t depot = 0;
						depot
								< this->getSubpopsPool().at(sId).getPairing().at(
										pId).getDepotRelation().size();
						++depot) {

					Individual ind = Individual(this->getProblem(),
							this->getConfig(), depot, -1);

					if (this->getSubpopsPool().at(sId).getPairing().at(pId).getDepotRelation().at(
							depot) < 0) {
						// Best
						ind = this->getSubpopsPool().at(depot).getBest();
					} else {
						ind =
								this->getSubpopsPool().at(depot).getIndividualsGroup().getIndividuals().at(
										this->getSubpopsPool().at(sId).getPairing().at(
												pId).getDepotRelation().at(
												depot));
					}
		...
		for (auto idInd = 0; idInd < this->getEliteGroup()->getPool().size();
				++idInd) {
...
void Community::evaluateSubpopsParallel() {
	...
	while (this->getProblem()->getMonitor().isTerminated() == false) {
		...
		for (typedef_vectorIntSize idSubpop = 0;
				idSubpop < this->getSubpops().size(); ++idSubpop) {

			printf("evaluateSubpopsParallel: Waiting subpop: %lu\n", idSubpop);
			this->getProblem()->getMonitor().getSubpopLock(idSubpop)->wait(
					true);
			...
			for (typedef_vectorIntSize idInd = 0;
					idInd
							< this->getSubpops().at(idSubpop).getIndividualsGroup().getIndividuals().size();
					++idInd) {
			...
			printf("evaluateSubpopsParallel: Notifying subpop: %lu\n",
					idSubpop);
			this->getProblem()->getMonitor().getSubpopLock(idSubpop)->notify(
					false);
...
void Community::evaluateSubpop(int subpopID) {
	...
	while (this->getProblem()->getMonitor().isTerminated() == false) {
		...
		for (typedef_vectorIntSize idInd = 0;
				idInd
						< this->getSubpops().at(subpopID).getIndividualsGroup().getIndividuals().size();
				++idInd) {
