...
void MDVRPProblem::processInstanceFiles(char* dataFile, char* solutionFile, char* instCode) {
    ...
    for (i = 0; i < this->getCustomers(); ++i) {
        fscanf(data, "%d %d %d %d %d", &type, &point.x, &point.y, &type, &valueInt);
    ...
    // Depot informations
    for (i = 0; i < this->getDepots(); ++i) {
        fscanf(data, "%d %d %d", &type, &point.x, &point.y);
